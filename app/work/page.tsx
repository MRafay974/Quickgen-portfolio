          ...
          {visibleCards.length === 0 ? (
            ...
            <>
              ...
              <div className="hidden sm:block">
                    ...
                    <div className="relative h-[380px] lg:h-[500px] w-full overflow-hidden bg-zinc-200">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-8 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                        ...
                        <p className="mt-1 text-sm font-semibold text-white/80">
                          {formatCategoryLabel(visibleCards[0].category)}
                        </p>
                      </div>
                    </div>
                  </article>
                ...
                {/* Masonry grid — natural image sizes, no forced aspect ratios */}
                {visibleCards.length > 1 && (
                  <div className="columns-2 gap-6">
                    {visibleCards.slice(1).map((project) => (
                      <Link
                        key={project.slug}
                        href={`/work/${project.slug}`}
                      ...
                      >
                        <article className="rounded-2xl border border-zinc-200 bg-zinc-50 shadow-sm transition hover:shadow-lg overflow-hidden">
                          {/* No overflow-hidden or fixed height on this wrapper — lets image breathe */}
                          <div className="relative">
                            {project.image ? (
                              // plain <img> renders at true natural dimensions, no Next.js cropping
                              // eslint-disable-next-line @next/next/no-img-element
                              <img
                                src={project.image}
                                alt={project.title}
